...
class policy(torch.nn.Module):
    def __init__(self):
        ...
        self.fc_input1 = torch.nn.Linear(input_features, layer_width)  # Common Input Layer
        self.fc_input2 = torch.nn.Linear(input_features, layer_width)  # Common Input Layer
    ...
    def forward(self, *inputs):
        if len(inputs) > 1:
            ...
            x = inputs[0]
        x = torch.nn.functional.selu(self.fc_input1(x))  # Common Input Layer
        # x2 = torch.nn.functional.relu6(self.fc_input1(x))  # Common Input Layer
...
# cl_system = System([mip_node, system])
...
trajectories = cl_system.forward(data)
...
#%% DataLoaders
train_data = DictDataset({'X': x_train.to(device), 'D': torch.zeros(num_data,nsteps,2).to(device)}, name='train')  # Split conditions into train and dev
dev_data = DictDataset({'X': x_dev[:num_dev_data,:,:].to(device), 'D': torch.zeros(num_dev_data,nsteps,2).to(device)}, name='dev')
...
logger = BasicLogger(args=None, savedir=f'nsteps_{nsteps}/log_sigmoid_N{nsteps}', verbosity=1, stdout=['train_loss', 'dev_loss', 'eltime'])
...
dists = d_tensor[:s_length+nsteps+1,:].unsqueeze(0)

dists = torch.zeros(dists.shape)
refs1_sim = torch.tensor([[[ref1]]]).repeat_interleave(s_length, dim=1)
refs2_sim = torch.tensor([[[ref2]]]).repeat_interleave(s_length, dim=1)
refs = torch.cat((refs1_sim,refs2_sim), dim=-1)
print(dists.shape)
...
fig1.tight_layout()
plt.grid()
fig1.savefig(f'nsteps_{nsteps}/sigmoid_no_dist.pdf', bbox_inches='tight')
...
# torch.save(trajectories, f"nsteps_{nsteps}/sigmoid_trajectories.pt")
