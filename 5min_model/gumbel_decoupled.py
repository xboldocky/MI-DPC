...
for nsteps in [10,15,20,25,30,40]:
    ...
    # layer_width = 120+(nsteps) # 120+20 for nstep 20 
    layer_width = 140 # 120+20 for nstep 20 
    # layer_width = 200
    # if nsteps == 10: layer_width=80
    class policy(torch.nn.Module):
        ...
        def forward(self, *inputs):
            ...
            #         self.temperature = self.temperature * torch.exp(torch.tensor([-0.0000001]) * (self.current_epoch+1)) if self.current_epoch < 400 else self.temperature
            #     except:
            #         pass
            ...
            # out_digits = torch.gumbel_softmax(self.fc_output_x2(x2), tau=temperature_coefficient, hard=True)
        ...
        # def parameters_without_t(self):
                # """Return model parameters excluding self.t."""
                # return [p for p in super().parameters() if p is not self.temperature]
    ...
    #     {'params': cl_system.nodes[0].callable.parameters_without_t(), 'lr': 0.0003},
    #     {'params': cl_system.nodes[0].callable.temperature, 'lr': 0.001}
    # ])
    logger = BasicLogger(args=None, savedir=f'authdata_nsteps_{nsteps}/log_gumbel_N{nsteps}', verbosity=1, stdout=['train_loss', 'dev_loss', 'eltime'])
    scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=0.7)
    # temp_optimizer = torch.optim.SGD([cl_system.nodes[0].callable.temperature], lr=0.01)
    trainer = Trainer(
        problem.to(device),
    ...
    torch.save(cl_system.state_dict(), f'authdata_nsteps_{nsteps}/gumbel_states_model.pt')
    ...
    # try:
    #     u1_opt = scipy.io.loadmat(f"optimal_5_min/Hp{nsteps}.mat")['u1']
    #     u2_opt = scipy.io.loadmat(f"optimal_5_min/Hp{nsteps}.mat")['u2']
    #     u3_opt = scipy.io.loadmat(f"optimal_5_min/Hp{nsteps}.mat")['u3']
    #     x1_opt = scipy.io.loadmat(f"optimal_5_min/Hp{nsteps}.mat")['x1']
    #     x2_opt = scipy.io.loadmat(f"optimal_5_min/Hp{nsteps}.mat")['x2']
    # except:
    ...
    fig1.tight_layout()
    # plt.grid()
    fig1.savefig(f'authdata_nsteps_{nsteps}/gumbel_states_eval.pdf', bbox_inches='tight')
    ...
    file_path = f"authdata_nsteps_{nsteps}/output_gumbel_nsteps.txt"
    ...
    torch.save(trajectories, f"authdata_nsteps_{nsteps}/gumbel_trajectories.pt")
