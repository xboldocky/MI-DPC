...
with torch.no_grad():
    ...
    # for nsteps in [20]:
        ...
        #     for i in range(s_length):
        #         # print(i)
        #         result_raw = loss.forward({'X': x_tensor[[j],i,:].unsqueeze(0),'U': u_tensor[[j],i,:].unsqueeze(0)})#['loss']
        #         result_optim_raw = loss.forward({'X': optim_x[[j],i,:].unsqueeze(0),'U': optim_u[[j],i,:].unsqueeze(0)})#['loss'] #### CHANGE INDEXING FOR OPTIMAL INSTANCES
