...
#%% Policy network architecture
        ...
        for instance in range(len(optimal_data.keys())):
            ...
            for i in range(s_length):
                ...
                with torch.inference_mode():
                    ...
                    u01 = mip_policy.forward(in_data)
                    u02 = rounding_node.forward({'X': in_data[:,:2]}| {'D': in_data[:,2:]} | {'U_relaxed': u01})
                    u = rounding_clip.forward(u02)
                    end = time.process_time()
                    el_time = end - start
                # u = {'U_clipped': u}
                x_list.append(x['X'])
                x = cl_system.nodes[3].forward(u |x| {'D': dists[0,i,:].view(1,-1)})
                u_list.append(u['U_clipped'])
        ...
        #     for i in range(s_length):
        #         # print(i)
        #         result_raw = loss.forward({'X': x_tensor[[j],i,:].unsqueeze(0),'U_clipped': u_tensor[[j],i,:].unsqueeze(0)})#['loss']
        #         result_optim_raw = loss.forward({'X': optim_x[[j],i,:].unsqueeze(0),'U_clipped': optim_u[[j],i,:].unsqueeze(0)})#['loss'] #### CHANGE INDEXING FOR OPTIMAL INSTANCES
        #         result = result_raw['loss']
        #         result_optim = result_optim_raw['loss']
        #         DPC_loss_list.append(result)
        #         optim_loss_list.append(result_optim)
        #         feas_list.append(result_raw['penalty_loss'])
        # #     DPC_loss_stack = torch.vstack(DPC_loss_list)
        # #     optim_loss_stack = torch.vstack(optim_loss_list)
        #     feas_stack = torch.vstack(feas_list)
        # #     total_dpc_loss.append(DPC_loss_stack)
        # #     total_optim_loss.append(optim_loss_stack)

        # dpc_loss = torch.vstack(DPC_loss_list).view(-1,1873)
        # dpc_loss_max = dpc_loss.mean(dim=1).max()
        # dpc_loss_min = dpc_loss.mean(dim=1).min()
        # optim_loss = torch.vstack(optim_loss_list).view(-1,1873)
        # optim_loss_max = optim_loss.mean(dim=1).max()
        # optim_loss_min = optim_loss.mean(dim=1).min()
        # dpc_loss_mean = dpc_loss.mean()
        # optim_loss_mean = optim_loss.mean()
        # #%%
        ...
        #%%
        list_out.append(['N:',nsteps,
                        #  'DPC_mean:',dpc_loss_mean.item(),
                        #  'DPC_max:', dpc_loss_max.item(),
                        #  'DPC_min:', dpc_loss_min.item(),
                        #  'Optim_mean:',optim_loss_mean.item(),
                        #  'Optim_max:', optim_loss_max.item(),
                        #  'Optim_min:', optim_loss_min.item(),
                        #  'Subopt:', (dpc_loss_mean.item()/optim_loss_mean.item())*100-100,
                        #  'Feas:', feas_stack.sum(),
                        #  'Params:', n_parameters,
                        'EvalTime:',time_tensor[1:].mean().item()])
        print(list_out)


        file_path = f"results_{method}.txt"
        ...
        #     # print(torch.all(x_tensor[:,i,0])<0.0 or torch.all(x_tensor[:,i,0])>8.4) 
        #     # print(torch.all(x_tensor[:,i,1])<0.0 or torch.all(x_tensor[:,i,1])>x2_max) 
        # # %%

        # #%%
        # # for n in [10,15,20,25,30,40]:

        # #     a = torch.load(f'authdata_nsteps_{n}/log_gumbel_N{n}/metrics.pt')
        # #     print(a)
        # # for n in [10,15,20,25,30,40]: