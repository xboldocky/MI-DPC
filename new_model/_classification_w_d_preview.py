...
# from src.func.layer import netFCasd
...
class policy(torch.nn.Module):
    ...
    def forward(self, *inputs):
        ...
        # out1 = sigmoid_scale(self.fc5(x), -0.5, input_energy_max)  # Identity
...
class rounding_network(torch.nn.Module):
    ...
    def forward(self, *inputs):
        ...
        out2 = sigmoid_scale(self.fc6(x), -3.0, 3.0) # softmax first discrete
        # out2 = relaxed_round(sigmoid_scale(self.fc6(x), u_int_min, u_int_max)) # softmax first discrete
        # out2 = relaxed_round(self.fc6(x)) # softmax first discrete
        return torch.cat((out1,out2), dim=1)
...
# rounding_threshold = roundThresholdModel(layers=rounding_nn, param_keys=['X','D'], var_keys=['U_relaxed'], output_keys=['U'], int_ind={'U_relaxed': -1}, continuous_update=False, name='round')
...
rounding_gumbel = roundGumbelModel(layers=rounding_nn, param_keys=['X','D'], var_keys=['U_relaxed'], output_keys=['U'], int_ind={'U_relaxed': -1}, continuous_update=True, name='round', temperature=10.0)
rg_result = rounding_gumbel.forward({'X':torch.rand(10,2), 'D':torch.rand(10,input_features-2),'U_relaxed': torch.rand(10,3)})
print(rg_result)
...
rounding_gumbel.eval()
cl_system = systempreview.PreviewSystem([mip_node, rounding_gumbel,system], preview_keys=['D'], preview_node_name=['mip_policy','round'])
cl_system.nsteps = nsteps # prediction horizon
# Minor test
test_input = {'X': torch.rand(10,1,nx),'R_relaxed': torch.rand(10,nsteps,nref),'D': torch.rand(10,nsteps,nd)}
...
optimizer = torch.optim.Adam(cl_system.parameters(), lr=0.0001)
trainer = Trainer(
    problem.to(device),
...
fig1.savefig('classification_states.pdf')
plt.show()
