...
from neuromancer.modules.functions import bounds_scaling as sigmoid_scale
...
layer_width = 60 #500 without bn works best
class policy(torch.nn.Module):
    ...
    def forward(self, *inputs):
        ...
        # out1 = sigmoid_scale(self.fc5(x), 0.0, input_energy_max)  # Identity
        out2 = sigmoid_scale(self.fc6(x), 0.0, 3.4) # softmax first discrete
        
        
        # out2 = relaxed_round(sigmoid_scale(self.fc6(x), u_int_min, u_int_max)) # softmax first discrete
...
#%% rounding_threshold structure


layers_rnd = netFC(input_dim=input_features+nu, hidden_dims=[60]*3, output_dim=nu)

class rounding_network(torch.nn.Module):
    def __init__(self, input_dim, layer_width):
        super(rounding_network, self).__init__()
        self.fc1 = torch.nn.Linear(input_dim, layer_width)  # input 2
        self.fc2 = torch.nn.Linear(layer_width, layer_width) 
        self.fc3 = torch.nn.Linear(layer_width, layer_width) 
        self.fc4 = torch.nn.Linear(layer_width, layer_width) 
        self.fc5 = torch.nn.Linear(layer_width, 2) # linear -> identity
    ...
    def forward(self, *inputs):
        ...
        out1 = sigmoid_scale(self.fc5(x), -1.0, input_energy_max)  # Identity
        out2 = sigmoid_scale(self.fc6(x), 0.0, 3.0) # softmax first discrete
        # out2 = relaxed_round(sigmoid_scale(self.fc6(x), u_int_min, u_int_max)) # softmax first discrete
        # out2 = relaxed_round(self.fc6(x)) # softmax first discrete
        return torch.cat((out1,out2), dim=1)
    
rounding_nn = rounding_network(input_dim=input_features+nu, layer_width=100)    

rounding_threshold = roundThresholdModel(layers=rounding_nn, param_keys=['X','D'], var_keys=['U_relaxed'], output_keys=['U'], int_ind={'U_relaxed': -1}, continuous_update=True, name='round', slope=20.0)
# rounding_threshold = roundThresholdModel(layers=layers_rnd, param_keys=['X','D'], var_keys=['U_relaxed'], output_keys=['U'], int_ind={'U_relaxed': -1}, continuous_update=True, name='round')
r_result = rounding_threshold.forward({'X':torch.rand(10,2)*10.0, 'D':torch.rand(10,input_features-2)*10.0,'U_relaxed': torch.rand(10,3)*10.0})
print(r_result)


# rounding_gumbel = roundGumbelModel(layers=rounding_nn, param_keys=['X','D'], var_keys=['U'], output_keys=['U_round'], int_ind={'U': -1}, continuous_update=False, name='round')
# rg_result = rounding_gumbel.forward({'X':torch.rand(10,2), 'D':torch.rand(10,input_features-2),'U': torch.rand(10,3)})
# print(rg_result)

# rounding_node = Node(lambda x,y,z:rounding_threshold.forward(x|y|z), ['X','D','U'], ['U_round'], name='round_node')

#%% System architecture
system = Node(ss_model, ['X', 'U', 'D'], ['X'], name='system')
importlib.reload(systempreview)
rounding_threshold.eval()
cl_system = systempreview.PreviewSystem([mip_node, rounding_threshold,system], preview_keys=['D'], preview_node_name=['mip_policy','round'])
cl_system.nsteps = nsteps # prediction horizon
# Minor test
test_input = {'X': torch.rand(10,1,nx),'R': torch.rand(10,nsteps,nref),'D': torch.rand(10,nsteps,nd)}
...
print(trajectories['U'])
rounding_threshold.train()
#%%
num_data = 12000
num_dev_data = 1200
batch_size = 1200
...
for _ in range(num_extensions):
    ...
    extended_tensors.append(randomized_tensor)
extended_tensor2_train = torch.cat(extended_tensors, dim=0)

d_train = torch.cat((d1_train__[:num_data,:,:],extended_tensor2_train[:num_data,:,:]), dim=-1)
...
d_dev = torch.cat((d1_dev_[:num_dev_data,:,:].swapaxes(1,2),extended_tensor2_train[:num_dev_data,:,:]), dim=-1)
...
optimizer = torch.optim.Adam(cl_system.parameters(), lr=0.001)
trainer = Trainer(
    problem.to(device),
...
# trajectories_c = {key: tensor.cpu() for key, tensor in trajectories.items()}
# print(loss.forward({'X': torch.rand(1,1,2, dtype=torch.float32),'U': torch.rand(1,1,3, dtype=torch.float32)})['loss'])

# result = loss.forward({'X':trajectories['X'][[0],0,:].unsqueeze(0),
#                        'U': trajectories['U'][[0],0,:].unsqueeze(0)})['loss']
...
DPC_loss = torch.vstack(DPC_loss_list)
optim_loss = torch.vstack(optim_loss_list)
...
ax[0,0].plot(torch.zeros(dists.size(1)-nsteps).to('cpu'))
ax[0,0].plot(x1_max*torch.ones(dists.size(1)-nsteps).to('cpu'))
ax[0,0].legend()
...
ax[1,0].plot(torch.zeros(dists.size(1)-nsteps).to('cpu'))
ax[1,0].plot(x2_max*torch.ones(dists.size(1)-nsteps).to('cpu'))
...
ax[3,1].plot(input_energy_max*torch.ones(dists.size(1)-nsteps).to('cpu'))
...
fig1.savefig('threshold_states.pdf')
plt.show()
