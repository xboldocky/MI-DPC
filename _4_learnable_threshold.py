...
from neuromancer.modules.functions import bounds_clamp

from utils.rnd import roundGumbelModel, roundThresholdModel, netFC
...
#%% Policy network architecture
    ...
    def clip_fn(x):
        return torch.cat((bounds_clamp(x[:,:-1],0.0,8.0), bounds_clamp(x[:,[-1]], 0.0, 3.)), dim=-1)

    rounding_clip = Node(clip_fn, ['U'], ['U_clipped'], name='round_clip')

    #%% System architecture
    system = Node(ss_model, ['X', 'U_clipped', 'D'], ['X'], name='system')
    cl_system = SystemPreview([mip_node, rounding_node, rounding_clip ,system],  
                            preview_keys_map={'D': ['mip_policy','round']},
    ...
    if __name__ == "__main__":
        ...
        torch.save(cl_system, f'training_outputs/lt/models/model_lt_N{nsteps}.pt')
        
        training_data = {}
        training_data['NTP'] = n_parameters; training_data['TT'] = training_time
        torch.save(training_data, f'training_outputs/lt/models/training_data_N{nsteps}.pt')
