...
import scipy, time
import importlib
from utils.softmax import gumbel_softmax
...
for nsteps in [10,15,20,25,30,35,40]:
    ...
    integers = torch.tensor([[0.0, 1.0, 2.0, 3.0]]).T
    nx = A.shape[0]
    nu = B.shape[1]
    nd = E.shape[1]  
    nref = 0
    ...
    class policy(torch.nn.Module):
        ...
        def forward(self, *inputs):
            ...
            x2 = self.bn2_x2(x2)
            out_digits = gumbel_softmax(self.fc_output_x2(x2), tau=temperature_coefficient, hard=True, enable_gumbels=self.enable_gumbels)
            out2 = torch.matmul(out_digits, integers)
    ...
    if __name__=='__main__':
        ...
        torch.save(cl_system, f'training_outputs/softmax/models/model_softmax_N{nsteps}.pt')
        
        training_data = {}
        training_data['NTP'] = n_parameters; training_data['TT'] = training_time
        torch.save(training_data, f'training_outputs/softmax/models/training_data_N{nsteps}.pt')
